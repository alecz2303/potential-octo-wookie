...
class AdminUsersController extends AdminController {
    ...
    public function getData()
    {
        ...
        ->add_column('actions', '<a href="{{{ URL::to(\'admin/users/\' . $id . \'/edit\' ) }}}" class="iframe button tiny">{{{ Lang::get(\'button.edit\') }}}</a>
                                @if($username == \'admin\')
                                @else
                                    <a href="{{{ URL::to(\'admin/users/\' . $id . \'/delete\' ) }}}" class="iframe button tiny alert">{{{ Lang::get(\'button.delete\') }}}</a>
                                @endif
            ')
    ...
    {
        // Validate the inputs
        //$validator = Validator::make(Input::all(), $user->ruleset());
        if(Input::get('password') === null && Input::get('password_confirmation')=== null){
            $input=array(
                'username'=>Input::get('username'),
                'email'=>Input::get('email'),
                'password'=>$user->getOriginal('password'),
                'password_confirmation'=>$user->getOriginal('password')
            );
            $validator = $user->isValid($input);
        }else{
            $input=array(
                'username'=>Input::get('username'),
                'email'=>Input::get('email'),
        ...
        if ($validator==true)
        {
            ...
            if(!empty($password)) {
                ...
                $user->password_confirmation=$user->getOriginal('password');
            }
