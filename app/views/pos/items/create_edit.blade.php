	...
	<form method="post" action="@if (isset($items)) {{ URL::to('pos/items' . $items->id . '/edit') }}@endif" autocomplete="off" data-abide>
		...
		<div class="row">
			<div class="switch round large-4 columns">
				...
				<input id="is_serialized" type="checkbox" name="is_serialized" {{{ Input::old('is_serialized', isset($customers->is_serialized) ?  "checked" : null) }}}>
				<label for="is_serialized"></label>
			</div>

			<div class="switch round large-4 columns">
				Eliminado
				<input id="deleted" type="checkbox" name="deleted" {{{ Input::old('deleted', isset($customers->deleted) ?  "checked" : null) }}}>
